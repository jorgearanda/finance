...
class PriceUpdater:
    def __init__(self, verbose=False):
        ...
        self.scraper = YahooScraper(verbose=verbose)

    def update(self):
        if self.verbose:
            print("===Updating prices===")
        self._record_prices(self.scraper.get_quotes(symbols=self._ticker_symbols()))
        if self.verbose:
    ...
    def _record_prices(self, quotes):
        symbol = None
        for quote in quotes:
            if symbol != quote.symbol and self.verbose:
